				...
				$last_id = $_GET['id'];
					...
					if($row1 = $result1->fetch_object()){
						...
						if($room1!="0"){
							...
							.$row1->day." คืน ราคา ".$row1->pay_room1." บาท"."<BR>";
						}
						if($room2!="0"){
							$s2 = "<BR>".$row1->name_room2." ".$room2." ห้อง "
							.$row1->day." คืน ราคา ".$row1->pay_room2." บาท"."<BR>";
						}
						if($room3!="0"){
							$s3 = "<BR>".$row1->name_room3." ".$room3." ห้อง "
							.$row1->day." คืน ราคา ".$row1->pay_room3." บาท"."<BR>";
						}
						if($room4!="0"){
							$s4 = "<BR>".$row1->name_room4." ".$room4." ห้อง "
							.$row1->day." คืน ราคา ".$row1->pay_room4." บาท"."<BR>";
						}
						$s = $s1.$s2.$s3.$s4;
						$_SESSION['id_book'] = $last_id;
						$paymoney = $row1->paymoney;
						$_SESSION['paymoney']=$paymoney;
					...
					<div class=\"row\">
						...
						<div class=\"col-sm-5\" >
							<p><B>วันที่เข้าพัก :</B> $checkin</p>
							<p><B>วันที่ออก :</B> $checkout</p>
							<p><B>ระยะเวลาที่เข้าพัก :</B> $row1->day คืน</p>

							<p><B>ราคารวม :</B> $paymoney บาท </p>
						</div>
						<div class=\"col-sm-12\" >
